function [resBead,resBeadCenter,resBeadRad] = beadMatchMulti( datIn, datMask, myRef, ofstx, ofsty, plotme, thrMulti, rPhi )
...
%     fprintf('Ntry: %d\n',nTry);
    ...
    % find local maximum -----
    matchMapBest = abs(sum(matchMapPhase,3));
    [~,radBest] = max(matchMap,[],3);
    ...
    if plotme
        ...
        figure;imshow(K1);
        K1 = cat(3,matchMapBest/max(matchMapBest(:)),datIn*0.5,datIn*0);
        figure;imshow(K1);
    end
